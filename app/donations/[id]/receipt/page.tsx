...
export default async function ReceiptPage({
  ...
  const rawDate =
    ...
    (donation.received_at ? String(donation.received_at).split('T')[0] : null)

  const receivedDate = rawDate
    ? new Date(
        `${rawDate.includes('T') ? rawDate : rawDate + 'T00:00:00'}`,
      ).toLocaleDateString(locale === 'en' ? 'en-GB' : 'th-TH', {
        dateStyle: 'long',
  ...
  const expiryDate = donation.expiry_date
    ? new Date(`${donation.expiry_date}T00:00:00`).toLocaleDateString(
    ...
    <main className="mx-auto w-full max-w-5xl px-4 py-8 text-slate-900 dark:text-white print:max-w-none print:bg-white print:px-0 print:py-0 print:text-black">
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3 print:hidden">
        <div className="flex items-center gap-3">
          <Link
            href="/donations"
            className="flex h-10 w-10 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
            title={locale === 'en' ? 'Back to donations' : 'กลับไปหน้ารายการ'}
          >
            ←
          </Link>
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 dark:text-white">
              {locale === 'en' ? 'Donation Receipt' : 'ใบรับของบริจาค'}
            </h1>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {locale === 'en'
                ? 'Official donation receipt for donor'
                : 'ใบรับของบริจาคอย่างเป็นทางการ'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Link
            href={`/donations/${id}/receipt/edit`}
            className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
          >
            {dict.common.edit}
          </Link>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">
                  Walai<span className="text-red-500">Track</span>
                </div>
        ...
        <div className="grid grid-cols-1 gap-4 bg-slate-50 px-8 py-6 dark:bg-slate-950 md:grid-cols-2">
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900">
            ...
            <dl className="space-y-3 text-sm">
              ...
              <div className="flex items-start justify-between gap-4">
                <dt className="text-slate-500 dark:text-slate-400">
                  {locale === 'en' ? 'Center' : 'ศูนย์รับบริจาค'}
                </dt>
                <dd className="text-right font-medium text-slate-900 dark:text-white">
                  {center?.name ?? '—'}
                </dd>
