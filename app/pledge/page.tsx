...
export default async function PledgePage({
  ...
  const t = {
    ...
    title1:
      dict?.pledge?.title1 ||
      (isEn ? 'Notify Your' : 'แจ้งความประสงค์'),

    title2:
      dict?.pledge?.title2 ||
      (isEn ? 'Donation' : 'บริจาค'),
    ...
    givingPower:
      ...
      (isEn
        ? 'Every gift is a great power'
        : 'ทุกการให้ คือ พลังที่ยิ่งใหญ่'),
    ...
    backHome:
      dict?.pledge?.backHome ||
      (isEn ? 'Back to Home' : 'กลับหน้าหลัก'),

    formTitle:
      dict?.pledge?.formTitle ||
      (isEn ? 'Pledge Donation' : 'แจ้งความประสงค์บริจาค'),
    ...
    successTitle:
      dict?.pledge?.successTitle ||
      (isEn ? 'Submitted Successfully!' : 'ส่งข้อมูลสำเร็จ!'),
    ...
    optional:
      dict?.pledge?.optional ||
      (isEn ? '(Optional)' : '(ถ้ามี)'),

    itemToDonate:
      dict?.pledge?.itemToDonate ||
      (isEn ? 'Item to Donate' : 'สิ่งที่อยากบริจาค'),

    itemPlaceholder:
      dict?.pledge?.itemPlaceholder ||
      (isEn
        ? 'Specify items to donate'
        : 'ระบุสิ่งของที่ต้องการบริจาค'),

    category:
      dict?.pledge?.category ||
      (isEn ? 'Category' : 'หมวดหมู่'),

    selectCategory:
      dict?.pledge?.selectCategory ||
      (isEn ? 'Select category' : 'เลือกประเภท'),

    quantity:
      dict?.pledge?.quantity ||
      (isEn ? 'Quantity' : 'จำนวน'),

    qtyPlaceholder:
      dict?.pledge?.qtyPlaceholder ||
      (isEn ? 'Enter amount' : 'ระบุจำนวน'),
    ...
    noteOptional:
      dict?.pledge?.noteOptional ||
      (isEn ? '(Additional details)' : '(รายละเอียดเพิ่มเติม)'),
    ...
    submitBtn:
      dict?.pledge?.submitBtn ||
      (isEn ? 'Submit Pledge' : 'ส่งคำร้อง'),
    ...
    feat2Title:
      isEn ? 'Pass Along Care' : 'ส่งต่อความห่วงใย',
    ...
    thankSub:
      isEn
        ? 'in spreading help'
        : 'ที่ส่งต่อความช่วยเหลือ',

    sysTrackTitle:
      isEn
        ? 'Transparent Tracking'
        : 'ระบบติดตามการบริจาคโปร่งใส',
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f5f1e8] font-sans text-slate-800 transition-colors duration-200 dark:bg-slate-950 dark:text-slate-100">
      ...
      <section className="relative min-h-[calc(100vh-64px)] overflow-hidden py-8 md:py-12">
        ...
        {/* ============================================================
            BACKGROUND IMAGE
            ใช้รูปจาก public/pledge-bg.jpg
        ============================================================ */}
        <div
          className="pointer-events-none absolute inset-0 bg-cover bg-center bg-no-repeat"
          style={{
            backgroundImage: "url('/pledge-bg.jpg')",
          }}
        />
        ...
        <div className="relative z-10 mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid items-center gap-8 lg:grid-cols-[1fr_1.3fr_1fr] lg:gap-10">
            ...
            <div className="hidden space-y-6 lg:block">
              ...
              {/* Left Art Box */}
              <div className="relative mt-6 h-[200px] w-full overflow-hidden rounded-2xl border border-white/60 bg-white/45 p-4 shadow-sm backdrop-blur-sm dark:border-slate-800 dark:bg-slate-900/50">
            ...
            <div className="hidden space-y-6 lg:block">
              ...
              {/* Right Art Box */}
              <div className="relative mt-4 h-[180px] w-full overflow-hidden rounded-2xl border border-white/60 bg-white/40 p-4 shadow-sm backdrop-blur-sm dark:border-slate-800 dark:bg-slate-900/50">
                ...
                <div className="flex h-full flex-col justify-between">
                  ...
                  <p className="rounded-lg bg-white/70 p-2.5 text-[10px] leading-relaxed text-slate-600 shadow-2xs backdrop-blur-xs dark:bg-slate-800/80 dark:text-slate-300">
                    {t.sysTrackDesc}
                  </p>
